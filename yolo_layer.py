...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target=None):
        if self.training:
            ...
            masked_anchors = self.get_masked_anchors()
            t0 = time.time()
            nB = output.data.size(0)
            nA = len(masked_anchors)/self.anchor_step
            nC = self.num_classes
            ...
            th    = th.view(-1).cuda()
            tconf = tconf.cuda()
            tcls  = tcls[cls_mask].long().view(-1).cuda()
    
            coord_mask = coord_mask.view(nB*nA*nH*nW).cuda()
            ...
            if False:
                ...
                print('             total : %f' % (t4 - t0))
            logging('%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, w %f, h %f, conf %f, cls %f, total %f' % (self.seen, nGT, nCorrect, nProposals, loss_x.item(), loss_y.item(), loss_w.item(), loss_h.item(), loss_conf.item(), loss_cls.item(), loss.item()))
            return loss
        else:
            masked_anchors = self.get_masked_anchors()
